...
Registro de consumo de servicios adicionales
...
-- 1. EVT_EnviarConfirmacionReserva: Genera confirmaciones para reservas próximas.
...
WHERE fecha_llegada BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 3 DAY)
AND estado = 'Pendiente';

-- 2. EVT_ActualizarTarifasTemporada: Actualiza tarifas automáticamente según cambios de temporada.
...
UPDATE tarifa t
JOIN temporada temp
ON t.ID_temporada = temp.ID_temporada
SET t.tarifa_base = t.tarifa_base * temp.factorr
WHERE CURDATE () BETWEEN temp.fecha_inicio AND temp.fecha_fin;

-- 3. EVT_VerificarLlegadasDelDia: Genera lista diaria de llegadas previstas.

CREATE TABLE llegadas_dia (
...
ON SCHEDULE EVERY 1 MINUTE
DO
INSERT INTO llegadas_dia (ID_reserva, ID_cliente, fecha_llegada)
SELECT r.ID_reserva, r.ID_cliente, r.fecha_llegada
FROM reserva r
WHERE r.fecha_llegada = CURDATE();


-- 4. EVT_LiberarReservasSinGarantia: Libera reservas sin garantía que no se confirmaron.
...
WHERE deposito = 0
AND fecha_llegada <= CURDATE()

SELECT * FROM reserva

-- 5. EVT_GenerarReporteOcupacion: Genera reportes periódicos de ocupación y pronósticos.
...
ON SCHEDULE EVERY 1 MINUTE
DO
INSERT INTO reporte_ocupacion (fecha, habitaciones_ocupadas, reservas_futuras)
SELECT 
    CURDATE(),
    (SELECT COUNT(*) FROM habitacion WHERE estado = 'Ocupada'),
    (SELECT COUNT(*) FROM reserva WHERE fecha_llegada > CURDATE());

SELECT * FROM reporte_ocupacion
